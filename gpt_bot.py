...
from logic import (
    ...
    cancel_appointment,
    get_available_dates_button_format,
    get_time_slots_button_format
)
...
def run_conversation(user_input):
    ...
    if msg.get("function_call"):
        ...
        if fn_name == "get_available_slots":
            ...
            formatted = display_dates(slots)
            button_data=formatted
            messages.append({
                "role": "function",
                "name": fn_name,
                "content": formatted
            ...
            # Check if a future appointment already exists
            ...
            time = user_data["preferred_time"]
   
            # Check if slot is valid
            if date not in slots or time not in slots[date]:
                available_dates = get_available_dates_button_format(slots)
                available_times = get_time_slots_button_format(slots.get(date, []))
                buttons=[{"label":i,"intent":i} for i in button_data]
                return {
                    "response": f"❌ The selected slot is unavailable.\nPlease pick from the available options.",
                    "buttons": {
                        "dates": buttons,
                    }
                }

            # If reschedule requested, cancel previous one
            if user_data.get("reschedule_flow"):
                cancel_appointment(user_data["phone"])

            # Proceed to confirm appointment
            success, result = confirm_appointment_with_api(
                user_data, client_id, country_code, timezone
            )
